...
/**
 * This class performs the skeletonization of an image by means of ImageJ library.
 * 
 * @author ChiarellaG
 *
 */
public class SkeletonizePreprocess extends AbstractPreprocess {
	 ...
	 */
	public SkeletonizePreprocess(PreprocessingType p) {
		super(p);
	}
	 ...
	 */
	public SkeletonizePreprocess(SkeletonizePreprocess p) {
		super(p);
	}

	@Override
	public ImagePlus performPreprocess(ImagePlus img, double... params) {
		img.setProcessor(img.getProcessor().convertToByte(true));
		IJ.run(img, "Skeletonize", "");
		return img;
