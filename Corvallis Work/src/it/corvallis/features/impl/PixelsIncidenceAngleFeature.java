...
/**
 * The class to compute the pixels incidence angle feature according to Samuel - Daramola - NOVEL FEATURE EXTRACTION TECHNIQUE FOR OFF-LINE SIGNATURE VERIFICATION SYSTEM. It extends the class {@link AbstractFeature}.
 * 
 * @author ChiarellaG
 *
 */
public class PixelsIncidenceAngleFeature extends AbstractFeature {
	 ...
	 */
	public PixelsIncidenceAngleFeature(FeatureType feat) {
		super(feat);
	}
	 ...
	 */
	public PixelsIncidenceAngleFeature(PixelsIncidenceAngleFeature f) {
		super(f);
	}
	
	@Override
	protected Object computeFeature(ImagePlus img, double... parameters) {
		double mean_angle = Filter.meanIncidenceAngle(img);
		if (Double.isFinite(mean_angle))
			return new double[]{mean_angle};
		else
