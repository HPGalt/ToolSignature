...
/**
 * The class to compute the WidthAreaRatioFeature feature. It extends the class {@link AbstractFeature}.
 * It computes the effective width over area ratio.
 * 
 * @author ChiarellaG
 *
 */
public class WidthAreaRatioFeature extends AbstractFeature {
	 ...
	 */
	public WidthAreaRatioFeature(FeatureType feat) {
		super(feat);
	}
	 ...
	 */
	public WidthAreaRatioFeature(WidthAreaRatioFeature f) {
		super(f);
	}
	...
	protected Object computeFeature(ImagePlus img, double... parameters) {
		...
		img = PreProcessingFactory.getPreProcess(PreprocessingType.HorizontalJoin).execute(img, new double[0]);
		return new double[] {(double) (img.getWidth()) / area};
	}
