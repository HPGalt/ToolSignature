...
/**
 * The class to compute the Area. It extends the class {@link AbstractFeature}.
 * The algorithm is provided by means of {@link Hu} moments.
 * 
 * @author ChiarellaG
 *
 */
public class AreaFeature extends AbstractFeature {
	 ...
	 */
	public AreaFeature(FeatureType feat) {
		super(feat);
	}
	 ...
	 */
	public AreaFeature(AreaFeature f) {
		super(f);
	}
	
	@Override
	protected Object computeFeature(ImagePlus img, double... parameters) {
		return new double[]{Hu.moment(img, 0, 0, 0, 0, img.getWidth(), img.getHeight()) / (double) ((img.getHeight()) * (img.getWidth()))};
	}
